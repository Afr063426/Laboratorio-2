...
require(xts)
require(YieldCurve)
datos<-read.csv('quarterly_csv.csv')
datos<-cbind(datos,seq(3,3*length(datos[,2]),by=3))
datos<-datos[c(1:20),]
```
...
time<-seq(3,3*20,by=3)

ggplot()+geom_line(aes(x=time,y=NS(parametros,time)))+geom_point(aes(
  x=seq(3,3*length(datos[,2]),by=3),y=datos[,2]/100))
