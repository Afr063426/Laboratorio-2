...
theta<-rep(0.2,4)
NS<-function(theta,T){
  yield<-theta[1]+(theta[2]+theta[3]/theta[4])*(1-exp(-theta[4])*T)/(theta[4]*T)-theta[3]/theta[4]*exp(-theta[4])*T
  yield
}

## Se debe ajustar la curva entonces esto se hace
fitNS<-function(theta,T,rendimientos){
  fn<-function(t){
    sum((rendimientos-NS(t,T))^2)
  }
  optim(theta,
        fn,
        method='L-BFGS-B',
...
##
T<-seq(from=1,to=7,by=1)
yi<-seq(0.01,0.07,length=7)
t<-c(rep(0,3),0.02)
fit<-fitNS(t,tiempo,yi)
ggplot()+geom_line(aes(x=tiempo,y=NS(,tiempo)))
```
