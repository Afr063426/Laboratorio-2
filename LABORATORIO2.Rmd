...
ggplot( data =  datosBrasil[datosBrasil$TipoDeBono=="Tesouro Prefixado",],mapping = aes(x =FechaDeMaduracion, y = Precio,group=FechaDeMaduracion, color=as.factor(FechaDeMaduracion))) +geom_boxplot(aes(color = as.factor(FechaDeMaduracion)), alpha = 0.7) +geom_jitter(aes(color = as.factor(FechaDeMaduracion)), size = 1, alpha = 0.04)+scale_color_discrete()+labs(y = "Tasa de rendimiento en %",x = "Tiempo",title = "Comportamiento de las tasas spot") +theme(legend.position="bottom")+theme(plot.title = element_text(hjust = .5))# colocamos el titulo en el centro
```
