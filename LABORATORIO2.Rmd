...
\begin{itemize}
\item El sacar un promedio de los precios de los datos con los que se cuenta es una opción para determinar la curva de rendimientos, sin embargo esto puede estar alejado de la realidad además de que se puede generar un gran error entre los datos reales de la media y lo obtenido mediante el modelo de Nelson-Siegel.
...
\item En los casos reales elaborados como los son tomar el máximo de los precios y con ello todas las valoraciones que se hicieron la fecha ligada este precio, así como el mínimo y el caso de la valoración más reciente que se tenía de los bonos, se nota que el error obtenido bajo el modelo de Nelson-Siegel y la forma en que se encontraron los parámetros es relativamente bajo ante la rapidez con la que se puede implementar esto.
A continuación se muestra un resumen de los resultados obtenidos.

\end{itemize}

``` {r echo=FALSE, message=FALSE, warning=FALSE,fig.cap="Fuente: Elaboración propia, con datos del Tesouro Nacional (2021)"}
...
\item Se concluye que a pesar de la rapidez con la que se puede implementar el modelo de Nelson-Siegel, se debería valorar la posibilidad de implementar otros modelos como el de Splines cubicos o Nelson-Siegel-Svensson, que podrían tener un menor error.
...
kable(datosEsta, format = "latex" , booktabs=TRUE , align = 'clcc' , caption = "Datos estadisticos",)%>% kable_styling(full_width = F, position = "center", latex_options = "HOLD_position")
```
