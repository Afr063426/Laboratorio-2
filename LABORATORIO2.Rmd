...
colnames(datosBrasil)<-c("Tipo_de_bono", "Fecha_de_vencimiento", "Dia_de_observacion", "Precio", "Tiempo_a_maduracion", "Cupones")
...
>>>>>>> a7a15af585ae0d2649a49cc3820f21ff4c4687f8
```
...
<<<<<<< HEAD
datosPrueba<-datosBrasil%>%filter(`Dia_de_observacion`==as.Date("27/05/2021",format="%d/%m/%Y"))
t<-c(0,0,0,0.5)
datosPrueba1<-datosPrueba%>%filter(!grepl("com",`Tipo_de_bono`))
datosPrueba1$Spot<-(datosPrueba1$Precio/1000)^(-1/datosPrueba1$`Tiempo_a_maduracion`)-1
parametros<-fitNS(t,datosPrueba$`Tiempo_a_maduracion`,datosPrueba$Precio,1000,datosPrueba$Juros/2)
=======
datosPrueba<-datosBrasil%>%filter(Data.Base==as.Date("27/05/2021",format="%d/%m/%Y"))
t<-c(0,0,0,0.03)
datosPrueba1<-datosPrueba%>%filter(!grepl("com",Tipo.Titulo))
datosPrueba1$Spot<-(datosPrueba1$PU.Base.Manha/1000)^(-1/datosPrueba1$TempoDeMaduracao)-1
parametros<-fitNS(t,datosPrueba$TempoDeMaduracao,datosPrueba$PU.Base.Manha,1000,datosPrueba$Juros)
>>>>>>> a7a15af585ae0d2649a49cc3820f21ff4c4687f8
tasasYield<-NS(unlist(parametros[1]),seq(0,10,length=100))
ggplot(,aes(x=seq(0,10,length=100),y=tasasYield))+geom_line()+geom_point(aes(x=datosPrueba1$`Tiempo_a_maduracion`,y=datosPrueba1$Spot))
```
...
\[
\exp\left(-\int_{0}^{T_i}r(t,\theta)dt\right)
\]
A la hora de emplearse esto es importante tener presente que se está incurriendo en un cierto grado de error con respecto al valor real que tendría el flujo en el presente.
\par
...
\begin{tcolorbox}
\par \textbf{Cuántil} Para $p\in [0,1]$ el $p$-ésimo cuantil $x_p$, para la función distribución $F_X$, se define como el menor número $x_p$ que satisface $p\leq F_X(x_p)$.
\end{tcolorbox}
...
\[
\var{x}=\frac{1}{n}\sum_{i=1}^{n}x_i
\]
Y la desviación estándar práctica es
\[
\sigma_x=\sqrt{\frac{1}{n-1}\sum_{i=1}}^{n}(x_i-\var{x})^2
\]
La media expresa alrededor de dónde se distribuyen los datos y la desviación estándar qué tanto se dispersan los datos con respecto a la media.
\end{tcolorbox}
...
Los datos son obtenidos a partir de la pa
Se parte de la base de datos que contiene las tasas de rendimiento, seleccionan sus ultimos 10 años, que van desde `r datosBrasil$"Dia_de_observacion"[1]` hasta `r datosBrasil$"Dia_de_observacion"[length(datosBrasil$"Dia_de_observacion")]` y cuenta con las tasas de rendimientos diarias de bonos con fecha de maduración a: 1 mes, 6 meses, 1 año, 2 años, 3 años, 5 años, 7 años, 10 años, 20 años y 30 años. En el siguiente grafico se aprecia como varia la tasa de rendimiento atreves del tiempo.
...
Como se puede observar de un periodo de tiempo tan amplio y contando con `r length(datosBrasil$"Dia_de_observacion")` observaciones por cada fecha de maduración, la tasa de rendimiento ha variado considerablemente, así que para realizar un análisis de sus datos se cuenta con el siguiente grafico de cajas y dispersión de puntos.
