...
datosPrueba<-datosBrasil%>%filter(FechaDeValoracion==as.Date("27/05/2021",format="%d/%m/%Y"))
t<-c(0,0,0,0.03)
datosPrueba1<-datosPrueba%>%filter(!grepl("com",TipoDeBono))
datosPrueba1$Spot<-(datosPrueba1$Precio/1000)^(-1/datosPrueba1$TiempoRestanteParaMaduracion)-1
parametros<-fitNS(t,datosPrueba$TiempoRestanteParaMaduracion,datosPrueba$Precio,1000,datosPrueba$CuponSemestral)
tasasYield<-NS(unlist(parametros[1]),seq(0,10,length=100))
ggplot(,aes(x=seq(0,10,length=100),y=tasasYield))+geom_line()+geom_point(aes(x=datosPrueba1$TiempoRestanteParaMaduracion,y=datosPrueba1$Spot))
