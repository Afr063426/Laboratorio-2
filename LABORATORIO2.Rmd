...
require(YieldCurve)
library(Quandl)
datos<-read.csv("Apple semanal.csv")
datos$ï..Date<-gsub(",","",datos$ï..Date)
datos<-datos[order(as.Date(datos$ï..Date),format="%B-%d-%Y"),]
```
...
t<-c(0,0,0,0.05)
fit2<-fitNS(t,datos1[,3],datos1[,2]/100)
fit<-fitNS(t,datos[,3],datos[,2]/100)
parametros<-as.numeric(unlist(fit[1]))
parametros2<-as.numeric(unlist(fit2[1]))

time<-seq(1,432,length=432)
ggplot()+geom_line(aes(x=datos[,3],y=NS(parametros,datos[,3])))+geom_point(aes(
  x=datos[,3],y=datos[,2]/100))

ggplot()+geom_line(aes(x=time,y=NS(parametros2,time)))+geom_point(aes(
  x=datos1[,3],y=datos1[,2]/100))
