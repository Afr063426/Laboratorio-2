...
Los datos son obtenidos a partir del Tesoro Nacional de la República Federativa de Brasil, esto por medio de su página web, la cual cuenta con una amplia descripción de los mismo. Se tiene que los mismo son bonos denominados \textit{pre-fixados} , con la característica de contar con rentabilidad fija en el momento del vencimiento. Los bonos \textit{pre-fixados} se dividen en bonos cuponados llamado “juros” los cuales cuentan con pagos semestrales y los bonos cero cupón, ambos pagan 1000 reales al vencimiento. Cabe destacar que tienen plazos fijos en sus bonos 


Se parte de la base de datos que contiene los precios de los bonos, seleccionando sus ultimos 10 años, que van desde `r datosBrasil$FechaDeValoracion[1]` hasta `r datosBrasil$FechaDeValoracion[length(datosBrasil$FechaDeValoracion)]` y cuenta con las tasas de rendimientos diarias de bonos con fecha de maduración a: 1 mes, 6 meses, 1 año, 2 años, 3 años, 5 años, 7 años, 10 años, 20 años y 30 años. En el siguiente grafico se aprecia como varia la tasa de rendimiento atreves del tiempo.
...
#Maximo<-datosCurva%>%filter(grepl("Max",Etiqueta))%>%ggplot(aes(x=Maduracion,y=Tasa))+geom_line(color="darkblue")
#Minimo<-datosCurva%>%filter(grepl("Min",Etiqueta))%>%ggplot(aes(x=Maduracion,y=Tasa))+geom_line(color="Orange")
#Media<-datosCurva%>%filter(grepl("Media",Etiqueta))%>%ggplot(aes(x=Maduracion,y=Tasa))+geom_line(color="Orange")
#Mediana<-datosCurva%>%filter(grepl("Mediana",Etiqueta))%>%ggplot(aes(x=Maduracion,y=Tasa))+geom_line(color="darkblue")
#ggarrange(Maximo,Minimo,Media,Mediana,labels = c("Max","Min","Media","Mediana"))
